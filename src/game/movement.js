...
export function Movement(sfx, objects, settings, rnd) {
    ...
    this.steer_player = function (p) {
        ...
        if (settings.jetpack == 0) {
            ...
            if (settings.pogostick == 1 || (p.jump_ready == 1 && p.action_up)) {
                ...
                /* jump */
                if (
                    GET_BAN_MAP_XY(s1, s2 + 16) == BAN_SOLID ||
                    GET_BAN_MAP_XY(s1, s2 + 16) == BAN_ICE ||
                    ...
                    p.jump_ready = 0;
                    p.jump_abort = 1;
                    if (settings.pogostick == 0) sfx.jump();
                    else sfx.spring();
                }
                /* jump out of water */
            ...
            if (p.action_up) {
                ...
                if (GET_BAN_MAP_IN_WATER(s1, s2)) p.in_water = 0;
                if (rnd(100) < 50)
                    objects.add(
                        objects.SMOKE,
                        (p.x.pos >> 16) + 6 + rnd(5),
        ...
        }
        {
            ...
            s1 = p.x.pos >> 16;
            if (
                GET_BAN_MAP_XY(s1, s2) == BAN_SOLID ||
                GET_BAN_MAP_XY(s1, s2) == BAN_ICE ||
                GET_BAN_MAP_XY(s1, s2) == BAN_SPRING ||
            ...
            s1 = p.x.pos >> 16;
            if (
                GET_BAN_MAP_XY(s1 + 15, s2) == BAN_SOLID ||
        ...
        s2 = p.y.pos >> 16;
        if (
            GET_BAN_MAP_XY(s1 + 8, s2 + 15) == BAN_SPRING ||
            (GET_BAN_MAP_XY(s1, s2 + 15) == BAN_SPRING &&
                GET_BAN_MAP_XY(s1 + 15, s2 + 15) != BAN_SOLID) ||
            (GET_BAN_MAP_XY(s1, s2 + 15) != BAN_SOLID &&
                GET_BAN_MAP_XY(s1 + 15, s2 + 15) == BAN_SPRING)
            ...
            for (var c2 = 0; c2 < env.MAX_OBJECTS; c2++) {
                ...
                if (obj.used == 1 && obj.type == objects.SPRING) {
                    if (GET_BAN_MAP_XY(s1 + 8, s2 + 15) == BAN_SPRING) {
                        if (
                            obj.x.pos >> 20 == (s1 + 8) >> LEVEL_SCALE_FACTOR &&
                            obj.y.pos >> 20 == (s2 + 15) >> LEVEL_SCALE_FACTOR
                        ) {
                            start_anim(obj);
                            break;
                        }
                        ...
                        if (
                            obj.x.pos >> 20 == s1 >> LEVEL_SCALE_FACTOR &&
                            obj.y.pos >> 20 == (s2 + 15) >> LEVEL_SCALE_FACTOR
                        ) {
                            start_anim(obj);
                            break;
                        ...
                        if (
                            obj.x.pos >> 20 == (s1 + 15) >> LEVEL_SCALE_FACTOR &&
                            obj.y.pos >> 20 == (s2 + 15) >> LEVEL_SCALE_FACTOR
                        ) {
                            start_anim(obj);
                            break;
        ...
        if (GET_BAN_MAP_XY(s1 + 8, s2 + 8) == BAN_WATER) {
            ...
            if (p.in_water == 0) {
                if (settings.bunnies_in_space == 0) p.y.velocity += 12288;
                else p.y.velocity += 6144;
    ...
    this.collision_check = function () {
        ...
        for (var c1 = 0; c1 < 3; c1++) {
            for (var c2 = c1 + 1; c2 < 4; c2++) {
                var pair = new Player_Pair(player[c1], player[c2], sfx, objects, settings);
    ...
    function player_action_left(p) {
        ...
        var is_ice_below =
            below == BAN_ICE ||
            (below_left != BAN_SOLID && below_right == BAN_ICE) ||
